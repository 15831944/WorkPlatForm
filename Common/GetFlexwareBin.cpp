...
CString GetFlexwareBinPathMethod()
{
	CString str;
	{
		CString sResult = LmPeraglobalPeraComponent();
		if (sResult) str = sResult;
	}
...
CString GetProcessServicePath()
{
	CString sPath = GetFlexwareBinPathMethod() + "\\PeraProcessService.exe";
	if (PathFileExists(sPath) != TRUE)
	{
		sPath.Empty();
	}
