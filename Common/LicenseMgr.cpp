...
CLicenseMgr theLicenseMgr;

BOOL InitLicense( LPCTSTR lpszFeatureName )
{
#ifndef _DEBUG
	if(!theLicenseMgr.CheckOutLicense(lpszFeatureName))
	{
		return FALSE;
	}
#endif
...
BOOL CLicenseMgr::multiToUnicode(const std::string& multiText, std::wstring& unicodeText)
{
...
BOOL CLicenseMgr::UnicodeTomulti(const std::wstring& unicodeText, std::string& multiText)
{
...
BOOL CLicenseMgr::multiToUTF8(const std::string& multiText, std::string& utf8Text)
{
	int size = ::MultiByteToWideChar(CP_ACP,0,multiText.c_str(),-1,NULL,0);
	if (0 == size)
	{
...
BOOL CLicenseMgr::UTF8Tomulti(const std::string& utf8Text, std::string& multiText)
{
...
BOOL CLicenseMgr::UTF8ToUnicode(const std::string& utf8Text, std::wstring& unicodeText)
{
...
BOOL CLicenseMgr::UnicodeToUTF8(const std::wstring& unicodeText, std::string& utf8Text)
{
...
BOOL CLicenseMgr::UnicodeToGB2312(const std::wstring& unicodeText, std::string& gb2312Text)
{
	int iTextLen = ::WideCharToMultiByte(CP_ACP, 0, unicodeText.c_str(), -1, NULL, 0, NULL, NULL);
	if (iTextLen <= 0)
	{
...
BOOL CLicenseMgr::GB2312ToUnicode(std::string& gb2312Text, std::wstring& unicodeText)
{
...
BOOL CLicenseMgr::CheckOutLicense(const char *feature)
{
	m_feature = feature;
	std::string strErr;
	int status = 0;
