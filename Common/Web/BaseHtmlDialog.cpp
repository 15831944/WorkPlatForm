	...
	, CComPtr<IHTMLWindow2> pHtmlWindow )
{
	...
	//////////////////////////////////////////////////////////////////////////
		CString str, strLog;
		str.Format("MethodName: %s\r\n", pMethodData->m_sMethodName);
		strLog += str;
		for (int i=0; i<(int)pMethodData->m_vParams.size(); i++)
		{
			str.Format("Param[%i]:\r\n    name: %s\r\n    value: %s\r\n"
				, i
				, pMethodData->m_vParams[i]->m_sName
				, pMethodData->m_vParams[i]->m_sValue);
			strLog += str;
		}
	...
	if ( pHtmlWindow == NULL )
	{
		...
		if ( pHtmlWindow == NULL )
		{
			::MessageBox( NULL, m_strLog, NULL, MB_OK );
			pMethodData->m_sInfo = m_strLog;
			m_strLog = "";
			return FALSE;
		}
	}
	...
	try
	{
		...
		strMethodNameW = pMethodData->m_sMethodName;
...

		{
			...
			{
				bstrTemp = _bstr_t(pMethodData->m_vParams[i]->m_sValue).GetBSTR();
			...
			//lc,国际化乱码问题
			pMethodData->m_sResult = (LPCTSTR)_bstr_t(varResult.bstrVal);

			str.Format("Result: %s\r\n", pMethodData->m_sResult);
			strLog += str;
...
BOOL CBaseHtmlDialog::WebHotKeyHandle( MSG* pMsg, HWND hHostWnd )
{
	if ( pMsg == NULL )
	{
		return FALSE;
...
// 				long lKeyData = (long) pMsg->lParam;
				::SetFocus( hHostWnd );
				::PostMessage( hHostWnd, pMsg->message, pMsg->wParam, pMsg->lParam );
			}
			break;
		...
		case WM_LBUTTONUP:
			if ( IsWindow( hHostWnd ) )
			{
				...
				if ( !(point.x == -3000 && point.y == -3000) ) 
				{
					//当前点为模拟鼠标点击的点时，不转发
					//解决执行环境在重新加载svg时，鼠标移动到最大化最小化关闭按钮时会触发事件问题
					::PostMessage( hHostWnd, pMsg->message, pMsg->wParam, pMsg->lParam );
				}
			}
			break;
