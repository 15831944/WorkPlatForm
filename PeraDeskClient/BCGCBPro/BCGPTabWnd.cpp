...
void CBCGPTabWnd::AdjustTabs ()
{
	...
	m_nTabsTotalWidth = 0;
	int nTabRectOffectX, nTabRectOffectY;
	CBCGPVisualManager::GetInstance ()->GetTabOffectInTabsArea_LocationTop(nTabRectOffectX, nTabRectOffectY);
	int nTabSpace = CBCGPVisualManager::GetInstance ()->GetTabSpaceInTabsArea();
	...
	for (int nIndex = 0; nIndex < m_iTabsNum; nIndex++)
	{
		...
		if (pTab->m_bVisible)
		{
			...
			else
			{
				...
				if ( pTab->m_nMinFullWidth > 0 && pTab->m_nFullWidth < pTab->m_nMinFullWidth )
				{
					pTab->m_nFullWidth = pTab->m_nMinFullWidth;
				}
			...
			if ( pTab->m_nMinFullWidth > 0 )
			{
			}
			else
			{
				if (IsActiveTabCloseButton() && i == m_iActiveTab)
				{
					pTab->m_nFullWidth += m_rectTabsArea.Height () - 2;
				}
			}
		...
		pTab->m_rect = CRect (CPoint (x, m_rectTabsArea.top),
						CSize (nTabWidth, m_rectTabsArea.Height () - nTabRectOffectY));
		...
		{
			pTab->m_rect.OffsetRect (nTabRectOffectX, nTabRectOffectY);
		}
		...
		x += pTab->m_rect.Width () + nTabSpace - nExtraWidth;
		m_nTabsTotalWidth += pTab->m_rect.Width () + nTabSpace;
	...
	else
	{
		...
		if (nCutTabsNum > 0)
		{
			...
			for (int nIndex = 0; nIndex < m_iTabsNum; nIndex++)
			{
				...
				pTab->m_rect = CRect (CPoint (x, m_rectTabsArea.top),
								CSize (nCurrTabWidth, m_rectTabsArea.Height () - nTabRectOffectY));
...
void CBCGPTabWnd::RecalcLayout ()
{
	...
	else
	{
		...
		{
			//m_rectWndArea.top = m_rectTabsArea.bottom + nTabBorderSize;
			//TabArea 与 WndArea 之间，无间距
			m_rectWndArea.top = m_rectTabsArea.bottom + nTabBorderSize - 1;
			m_rectWndArea.left++;
		}
	}
...
BOOL CBCGPTabWnd::SetTabMinFullWidth (int iTab, int nWidth)
{
	ASSERT_VALID (this);
	...
	if ( nWidth < 0 )
		pTab->m_nMinFullWidth = 0;
	else
	{
		pTab->m_nMinFullWidth = nWidth;
	}
