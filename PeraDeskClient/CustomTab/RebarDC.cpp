...
class XThemeManager: public CCxBCGPVisualManager2007
{
...
void CMemDC2::SetSize (const CSize& size)
{
	if (m_DC.GetSafeHdc () == NULL)
	{
		CreateDC ();
...
void CRebarDC::SetSize (const CSize& size)
{
	...
	if (m_Bitmap.GetSafeHandle () != NULL)
	{
		...
		CBCGPDrawManager dm (m_DC);
		COLORREF clr1 = RGB(252,252,252);
		COLORREF clr2 = RGB(134,173,231);
