...
//Tab间隔 10.10.11 added
const int n_SizeSpanX    = 2;
...
const int n_SizeTabMinX  = 68;
const int n_SizeTabMid1X = 100;
const int n_SizeTabMid2X = 120;
const int n_SizeTabMaxX  = 160;
const int n_SizeTabSMinX = 90;
...
void CTabMenuButton::OnShowMenu ()
{
	...
	if (bRes)
	{
		//如果当前活动view是Flexware，则Tabbar左侧的小按钮菜单，显示不出来。
		//在此处作处理？
		CMainFrame * pMainWnd = (CMainFrame *)theApp.m_pMainWnd;
//		CPeraWsView * pView = (CPeraWsView *) pMainWnd->GetViewPtrById(theApp.m_strFlexwareViewId);
		//CRunApp * pApp = NULL;
		//if (pView != NULL)
		//{
		//	pApp = (CRunApp *)pView->m_dwDataEx;
		//	if (pApp != NULL)
		//	{
		//		if (IsWindow(pApp->m_hwndApp))
		//		{
		//			//::SetWindowLong(pApp->m_hwndApp, GWL_STYLE, ::GetWindowLong(pApp->m_hwndApp, GWL_STYLE) | WS_CHILD);
		//			//::SetForegroundWindow(pMainWnd->GetSafeHwnd());
		//			//::SetWindowPos(pMainWnd->GetSafeHwnd(), HWND_TOP, 0,0,0,0, SWP_NOSIZE|SWP_NOMOVE|SWP_SHOWWINDOW);
		//		}
		//	}
		//}
...
void CTabWnd::ActivateMDI(int index)
{
	if (index != -1 && index != m_Active)
	{
		...
		if (pFrame != NULL)
		{
			pFrame->PostMessage (WM_MDIACTIVATE, (WPARAM)m_Tabs[index]->m_hWnd, TRUE);
		}
	}
}
...
//	if (CBCGPWinXPVisualManager::IsWinXPThemeAvailible ())
	...
	if (count > 0)
	{
		...
		//{
		//	bNeedMenu = TRUE;
		//	m_rectTabs.left += n_SizeMenuX + n_SizeSpanX;
		//}
		...
		else 
		{
			...
			m_rectTabs.right = m_rectTabs.left + min (m_rectTabs.Width (), (n_SizeTabMid2X + n_SizeSpanX) * count);
		}
...
void CTabWnd::AdjustTabs ()
{
	...
	if (count > 0)
	{
		...
		for (int i = 0; i < count; i++)
		{
			...
			rect.OffsetRect (rect.Width () + n_SizeSpanX, 0);
		}
...
void CTabWnd::DrawTab(CDC* pDC, CRect rect, int index)
{
	...
	pDC->DrawText (pTabInfo->m_strText, rect, 
		DT_END_ELLIPSIS | DT_SINGLELINE | DT_EXPANDTABS | DT_NOPREFIX | DT_CENTER | DT_VCENTER); //tab 文字居中 2012.09.04
	...
	if (m_rectTabs.left < m_rectTabs.right)
	{
		...
		for (int i = 0; i < count; i++)
		{
			...
			if (bWinXP)
			{
				...
				rectTab.top -= 4;
				//rectTab.DeflateRect(1,-1);//10.10.11
...
void CTabWnd::SetActiveTab (int nPos)
{
	if (nPos < 0 || nPos >= m_Tabs.GetSize ()) return;
	ActivateMDI(nPos);
}
...
void CTabWnd::OnLButtonDblClk(UINT nFlags, CPoint point)
{
	...
	if (pWnd != NULL)
	{
		int nTabIdx = GetTabIndexFromPoint(point);
		if (nTabIdx >= 0 && nTabIdx < m_Tabs.GetSize ())
		{
			CTabInfo* pTab = m_Tabs [nTabIdx];
			CString strTabName = pTab->m_strText;

			pWnd->SendMessage(WM_TAB_DOUBLE_CLICKED, (WPARAM)nTabIdx, (LPARAM)(LPCTSTR)strTabName);
		}
	}
