		...
		if (!bRibbonCaption)
		{
			...
			if (pMainFrame)
			{
				DrawNcCaption (&dc, rectCaption, dwStyle, dwStyleEx, 
					g_lpszAppTitle, strDocument, hIcon, bPrefix, bActive, m_bNcTextCenter,
					lstSysButtons);
			}
			else
			{
						...
						COLORREF clrText)
{
	...
	if (pTabWnd->IsActiveTabCloseButton () && bIsActive)
	{
		...
		CRect rectClose;
		pTabWnd->GetTabCloseButtonRect(pTabWnd->GetActiveTab(),rectClose,&bIsHighLight);
		...
		if ( /*pTabWnd->IsTabCloseButtonHighlighted ()*/ bIsHighLight)
		{
