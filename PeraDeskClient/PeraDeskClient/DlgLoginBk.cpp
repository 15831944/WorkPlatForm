...
BOOL CDlgLoginBk::OnInitDialog()
{
	...
	SetProp(m_hWnd,"PeraDeskClient_LoginWnd",HANDLE(1));
...
void CDlgLoginBk::OnDestroy()
{
	CDialog::OnDestroy();

	// TODO: 在此处添加消息处理程序代码
	RemoveProp(m_hWnd,"PeraDeskClient_LoginWnd");
}


BOOL CDlgLoginBk::OnCopyData(CWnd* pWnd, COPYDATASTRUCT* pCopyDataStruct)
{
	// TODO: 在此添加消息处理程序代码和/或调用默认值
	...
	{
	case CPMSG_LOGIN_DESKCLIENT:
		LoginFromCopyData(pCopyDataStruct->lpData);
		break;
	default:
		break;
	}
...
void  CDlgLoginBk::LoginFromCopyData(LPVOID lpData)
{
	...
	theApp.m_cxPeraDCData.m_sUser = data.m_sRealName;
	theApp.m_cxPeraDCData.m_sTicket = data.m_sTicket;
	theApp.m_cxPeraDCData.m_sRealName = data.m_sRealName;
	theApp.m_cxPeraDCData.m_sUrl = data.m_sUrl;

	if (theApp.m_pMainWnd && theApp.m_pMainWnd->m_hWnd
		&& ::IsWindowVisible(theApp.m_pMainWnd->m_hWnd)==FALSE)
	{ //主窗体隐藏了
