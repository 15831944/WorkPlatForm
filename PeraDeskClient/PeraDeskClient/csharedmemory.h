...
// Pointer auf gemeinsamen Speicher
	...
	//Constr ohne Init
	CSharedMemory(){csName = cstrSMName;m_bInit = FALSE;m_bAlreadyExist = FALSE;csMutexName = csName + "Mutex";}
	CSharedMemory(LPCSTR cs){csName = cs;m_bInit = FALSE;m_bAlreadyExist = FALSE;csMutexName = csName + "Mutex";}
	//Constr mit Init
	CSharedMemory(int size){csName = cstrSMName;m_bAlreadyExist = FALSE;csMutexName = csName + "Mutex";Init(size);}
