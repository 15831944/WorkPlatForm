...
// PeraPRCacheService.cpp : 定义应用程序的类行为。
//
...
// 唯一的一个 CPeraPRCacheServiceApp 对象

CPeraPRCacheServiceApp theApp;


// CPeraPRCacheServiceApp 初始化
...
BOOL CPeraPRCacheServiceApp::InitInstance()
{
	...
	{
		ZTools::WriteZToolsLog( "已经存在一个应用程序，本次启动失败" );
		CloseHandle( hMutex );
		hMutex = NULL;
		return FALSE;
	}
