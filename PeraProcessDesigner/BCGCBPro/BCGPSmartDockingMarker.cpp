...
void CBCGPSmartDockingMarker::Show (BOOL bShow)
{
	if (::IsWindow(m_wndBmp.m_hWnd))
	{
		...
		if (bShow)
		{
			SetWindowPos(m_wndBmp.GetSafeHwnd(), HWND_TOPMOST, 0, 0, 0, 0, SWP_NOMOVE|SWP_NOSIZE|SWP_SHOWWINDOW);
		}
		else
		{
			SetWindowPos(m_wndBmp.GetSafeHwnd(), HWND_NOTOPMOST, 0, 0, 0, 0, SWP_NOMOVE|SWP_NOSIZE|SWP_HIDEWINDOW);
		}
	}
}
...
void CBCGPSDCentralGroup::Show (BOOL bShow)
{
	if (::IsWindow(m_Wnd.m_hWnd))
	{
		...
		if (bShow)
		{
			SetWindowPos(m_Wnd.GetSafeHwnd(), HWND_TOPMOST, 0, 0, 0, 0, SWP_NOMOVE|SWP_NOSIZE|SWP_SHOWWINDOW);
		}
		else
		{
			SetWindowPos(m_Wnd.GetSafeHwnd(), HWND_NOTOPMOST, 0, 0, 0, 0, SWP_NOMOVE|SWP_NOSIZE|SWP_HIDEWINDOW);
		}
	}
}
