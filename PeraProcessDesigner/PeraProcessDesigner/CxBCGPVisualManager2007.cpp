		...
		if (!bRibbonCaption)
		{
			...
			if (pMainFrame)
			{
				DrawNcCaption (&dc, rectCaption, dwStyle, dwStyleEx, 
					strText, strDocument, hIcon, bPrefix, bActive, m_bNcTextCenter,
					lstSysButtons);
			}
			else
			{
						...
						BOOL bNCArea/* = FALSE*/)
{
	...
	//////////////////////////////////////////////////////////////////////////
	if (
		pBar->IsKindOf(RUNTIME_CLASS(CBarStructTree)) 
		|| pBar->IsKindOf(RUNTIME_CLASS(CBarResLib))
		)
	{
		if ( bNCArea )
		{
			COLORREF crBorder = g_crBarBorderOut;
			CBCGPDrawManager dm (*pDC);
			//左
			dm.DrawLine( 0, 0, 0, rectClient.bottom - 1, crBorder );
			//右
			dm.DrawLine( rectClient.right - 1, rectClient.top, rectClient.right - 1, rectClient.bottom - 1, crBorder );
			//下
			dm.DrawLine( 0, rectClient.bottom - 1, rectClient.right - 1, rectClient.bottom - 1, crBorder ); 
	...
	if (
		pBar->IsKindOf (RUNTIME_CLASS (CCxBCGPToolBar))
		|| pBar->IsKindOf (RUNTIME_CLASS (CCxRobotBar))
		|| pBar->IsKindOf (RUNTIME_CLASS (CCxLoginBar))
		)
	{
		CBCGPDrawManager dm (*pDC);
		dm.DrawRect( rectClient, g_crToolbarBg, g_crToolbarBg );
...
void CCxBCGPVisualManager2007::OnDrawBarGripper (CDC* pDC, CRect rectGripper, BOOL bHorz, CBCGPBaseControlBar* pBar)
{
    ...
    if (szBox != CSize (0, 0))
    {
		...
		CCxRobotBar* pRobotBar = DYNAMIC_DOWNCAST (CCxRobotBar, pBar);
		CCxLoginBar* pLoginBar = DYNAMIC_DOWNCAST (CCxLoginBar, pBar);
		CCxBCGPToolBar*  pXToolBar = DYNAMIC_DOWNCAST (CCxBCGPToolBar, pBar);

		if(pRobotBar || pLoginBar || pXToolBar)
		{
			nBoxesNumber ++;
			nOffset --;
		}
											...
											BOOL bSelected, BOOL bActiveItem, BOOL bSortedColumn)
{
	...
	BOOL bJianmoTree = pCtrl->IsKindOf(RUNTIME_CLASS(CCxStructTreeGridT));
...
BOOL CCxBCGPVisualManager2007::ParseStyleXMLMainWnd(const CString& strItem)
{
	...
	if (tmItem.ExcludeTag (_T("CAPTION"), strCaption))
	{
		...
		logfont.lfHeight   =   -13;      //设置字体的大小
		strcpy_s(logfont.lfFaceName,"宋体");
		m_AppCaptionFont.DeleteObject ();
		m_AppCaptionFont.CreateFontIndirect (&logfont);
						...
						int iTab, BOOL bIsActive, const CBCGPBaseTabWnd* pTabWnd)
{
	...
	static 	Image * pImgSel = LoadImage(GetExecDir() + "\\Res\\CoreImgs\\MainTabSelect.png");
	static 	Image * pImgNor = LoadImage(GetExecDir() + "\\Res\\CoreImgs\\MainTabNormal.png");
	static 	Image * pImgOver = LoadImage(GetExecDir() + "\\Res\\CoreImgs\\MainTabOverNormal.png");
	...
	else
	{
		...
		RectF rcDraw = RectF((float)rectTab.left, (float)rectTab.top, (float)rectTab.Width(), (float)rectTab.Height());

		if ( bIsActive )
		{
			DrawSudoku(pImgSel, &g, rcDraw);
		}
		else if ( !bIsActive && bIsHighlight )
		{
			DrawSudoku(pImgOver, &g, rcDraw);
		}
		else
		{
			DrawSudoku(pImgNor, &g, rcDraw);
		}
										 ...
										 const CBCGPBaseTabWnd* pTabWnd)
{
	...
	CBCGPTabWnd * pTab = theViewMgr.GetViewTab("建模环境");
	if ( pTab != NULL )
	{
		int x = GetTabsMargin(pTab) - 1;
		dm.DrawLine( rect.left + x, rect.top + 1, rect.left + x, rect.bottom, g_crTabBarLeftLine );
	}
}
...
BOOL CCxBCGPVisualManager2007::OnEraseTabsFrame (CDC* pDC, CRect rect, const CBCGPBaseTabWnd* pTabWnd)
{
	...
	if ( pTab != NULL )
	{
		...
		CRect rcTab, rcAera;
		pTab->GetTabsRect( rcTab );
		rcAera = rect;
		rcAera.top = rcTab.bottom;
		dm.DrawRect( rcAera, g_crTabBarBg2, g_crTabBarBg2 );
		return TRUE;
	}
						...
						COLORREF clrText)
{
	...
	static 	Image * pImgClose = LoadImage(GetExecDir() + "\\Res\\CoreImgs\\MainTabClose_Normal.png");
	static 	Image * pImgCloseOver = LoadImage(GetExecDir() + "\\Res\\CoreImgs\\MainTabClose_Focus.png");
	...
	if (pTabWnd->IsActiveTabCloseButton () && bIsActive)
	{
		...
		pTabWnd->GetTabCloseButtonRect(pTabWnd->GetActiveTab(),rectClose,&bHighLight);
		...
		Graphics g(pDC->GetSafeHdc());
		RectF rcDraw = RectF((float)rectClose.left, (float)rectClose.top, (float)rectClose.Width(), (float)rectClose.Height());

		if ( /*pTabWnd->IsTabCloseButtonHighlighted ()*/ bHighLight)
		{
			g.DrawImage( pImgCloseOver, rcDraw, 0.0f, 0.0f
				, (float)pImgCloseOver->GetWidth(), (float)pImgCloseOver->GetHeight(), Gdiplus:: UnitPixel );
		}
		else
		{
			g.DrawImage( pImgClose, rcDraw, 0.0f, 0.0f
				, (float)pImgCloseOver->GetWidth(), (float)pImgCloseOver->GetHeight(), Gdiplus:: UnitPixel );
		}
	}
...
{
	ZToolTipEvent* pTTE = dynamic_cast<ZToolTipEvent*>(pWnd);
	CMainFrame* pMainFrame = dynamic_cast<CMainFrame*>(pWnd);
	CBCGPDialog* pDlg = dynamic_cast<CBCGPDialog*>(pWnd);

	if (!pMainFrame && (!pDlg || pDlg->IsVisualManagerNCArea() == FALSE))
	{
	...
	for (POSITION pos = lstSysButtons.GetHeadPosition (); pos != NULL;)
	{
		...
		if(pTTE)
		{
			pTTE->m_pLstSysButtons = &lstSysButtons;
			CToolTipCtrl*& pTTC = pTTE->m_pToolTip;
			if(!pTTC)
			{			
				if(CBCGPTooltipManager::CreateToolTip (pTTC, pWnd, BCGP_TOOLTIP_TYPE_DOCKBAR))
				{
					for(int i = 0; i < lstSysButtons.GetSize(); i++)
					{
						CRect rectDummy;
						rectDummy.SetRectEmpty ();
						pWnd->GetWindowRect(rectDummy);
						pWnd->ScreenToClient(rectDummy);
						//pTTC->AddTool (pWnd, "自动隐藏", 0, 0);
						pTTC->AddTool (pWnd, "", rectDummy, i + 1);
					}			
				}		
			}
			CRect rectTT = pButton->GetRect ();
			//rectTT.OffsetRect(rectCaption.TopLeft());
			rectTT.OffsetRect(rcBar.TopLeft());
			//pWnd->ScreenToClient(rectTT);
			pTTC->SetToolRect (pWnd, i + 1, rectTT);
			//pTTC->SetToolRect(pWnd, i + 1, pButton->GetRect());
			CString str = "";
			const DWORD dwStyle = pWnd->GetStyle ();
			BOOL bMaximized = (dwStyle & WS_MAXIMIZE) == WS_MAXIMIZE;
			...
			case HTMAXBUTTON_BCG:
				str = (dwStyle & WS_MAXIMIZE) == WS_MAXIMIZE ? "还原" : "最大化";
				break;
			}
			pTTC->UpdateTipText(str, pWnd, i + 1);
		}
		i++;
	}
