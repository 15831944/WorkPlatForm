...
CCxStructTreeNodeFlow::CCxStructTreeNodeFlow( CCxStructTree * pTree )
...
CCxStructTreeNodeFlow::~CCxStructTreeNodeFlow()
{

}

void CCxStructTreeNodeFlow::Clear( void )
{
	m_strAuthor = "";
	m_strDescription = "";
	m_strCreated = "";
...
BOOL CCxStructTreeNodeFlow::OnParsing( Json::Value& _jsonObj )
{
...
CString CCxStructTreeNodeFlow::GetTooltipText( void )
{
	CString strTip;
	strTip.Format( "名称：%s\r\n作者：%s\r\n时间：%s\r\n版本：%s\r\n描述：%s"
		, m_strOriName
...
CString CCxStructTreeNodeFlow::GetUpdatePropJsonStr( void )
{
	try
	{
		CString strResult;
...
如果是根节点
*/
BOOL CCxStructTreeNodeFlow::CanRun()
{
...
void CCxStructTreeNodeFlow::OnAfterRunCompleted( void )
{
	CCxStructTreeNode* pNode = NULL;
	CCxStructTreeNodeParam* pParamNode = NULL;
	...
	for ( int i=0; i<(int)m_vChildren.size(); i++ )
	{
		...
		{
			((CCxStructTreeNodeGroup*)pNode)->GetParamNodes( vParams );
		}
		...
		{
			((CCxStructTreeNodeFlow*)pNode)->OnAfterRunCompleted();
		}
	}
