...
int CDlgResLib::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
	...
	CCreateContext ccc; 
	ccc.m_pNewViewClass      = RUNTIME_CLASS(CCxSplitterView); 
	...
	m_pFrameWnd = new CCxSplitterFrameWnd(); 
	...
	m_wndSplitter.CreateView (0, 0, RUNTIME_CLASS (CCxSplitterView)
		, CSize (100, 100), &ccc);

	theApp.m_nSpliterViewFlag = theApp.SPLVF_COMPONENT_LIST;
	m_wndSplitter.CreateView (0, 1, RUNTIME_CLASS (CCxSplitterView), 
		CSize (200, 100), &ccc);
	...
	theApp.m_nSpliterViewFlag = 0;

	//m_pFrameWnd->m_pSplitter = &m_wndSplitter;
	//m_wndSplitter.SubclassWindow(m_pFrameWnd->GetSafeHwnd());
