// DlgStructTreePropFlow.cpp : 实现文件
//
...
BEGIN_MESSAGE_MAP(CDlgStructTreePropFlow, CDialogEx)
	ON_BN_CLICKED(IDOK, &CDlgStructTreePropFlow::OnBnClickedOk)
	//ON_BN_CLICKED(IDCANCEL, &CDlgStructTreePropFlow::OnBnClickedCancel)
	ON_BN_CLICKED(IDC_STPF_OK, &CDlgStructTreePropFlow::OnBnClickedStpfOk)
	ON_BN_CLICKED(IDC_STPF_CANCEL, &CDlgStructTreePropFlow::OnBnClickedStpfCancel)

END_MESSAGE_MAP()

void CDlgStructTreePropFlow::DoDataExchange(CDataExchange* pDX)
{
	CDialogEx::DoDataExchange(pDX);
	DDX_Control(pDX, IDC_STPF_NAME, m_wndName);
	DDX_Control(pDX, IDC_STPF_AUTHOR, m_wndAuthor);
	DDX_Control(pDX, IDC_STPF_CREATED, m_wndCreated);
	DDX_Control(pDX, IDC_STPF_VERSION, m_wndVersion);
	DDX_Control(pDX, IDC_STPF_DESCRP, m_wndDescrp);
	DDX_Control(pDX, IDC_STPF_OK, m_wndOk);
	DDX_Control(pDX, IDC_STPF_CANCEL, m_wndCancel);
}

IMPLEMENT_DYNAMIC(CDlgStructTreePropFlow, CDialogEx)

CDlgStructTreePropFlow::CDlgStructTreePropFlow(CWnd* pParent /*=NULL*/)
	: CDialogEx(CDlgStructTreePropFlow::IDD, pParent)
{
	m_pNode = NULL;
}

CDlgStructTreePropFlow::~CDlgStructTreePropFlow()
{
}

void CDlgStructTreePropFlow::OnBnClickedOk()
{
}


void CDlgStructTreePropFlow::OnBnClickedCancel()
{
}


BOOL CDlgStructTreePropFlow::OnInitDialog()
{
...
void CDlgStructTreePropFlow::OnBnClickedStpfCancel()
{
	EndDialog(FALSE); return;
}

// Edit控件字串过滤
bool CDlgStructTreePropFlow::CheckEdit( CEdit& edit, STRATEGY strategy /*= STRATEGY_DEFAULT */, CString cName /*= ""*/,int nLenLimit /*= 20*/ )
{
	CString str;
	BOOL b;
